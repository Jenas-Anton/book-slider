...
const Page = ({
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < bones.length; i++) {
      ...
      const turningTime = Math.min(400, new Date() - turnedAt.current) / 400;
      const turningIntensity =
        Math.sin(i * Math.PI * (1 / bones.length)) * Math.sin(turningTime * Math.PI);
      ...
      const foldIntensity =
        i > 8 ? Math.sin(i * Math.PI * (1 / bones.length) - 0.5) * turningIntensity : 0;
...
export const Book = ({ pictures = [], params = DEFAULT_EASING, ...props }) => {
  ...
  const pages = useMemo(() => {
    const generated = [];
    if (pictures.length === 0) return generated;

    generated.push({ front: "book-cover", back: pictures[0] });
    ...
    generated.push({ front: pictures[pictures.length - 1], back: "book-back" });
    return generated;
  }, [pictures]);
